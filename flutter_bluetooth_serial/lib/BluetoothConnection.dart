...
class BluetoothConnection {
  ...
  Future<void> finish() async {
    try {
      // Verifique se a conexão está ativa antes de tentar enviar dados
      if (output != null && await output.isConnected) {
        await output.allSent;  // Espera até que todos os dados sejam enviados
      } else {
        throw StateError("Não há conexão ativa");
      }
    } catch (e) {
      print("Erro ao tentar enviar dados: $e");
    } finally {
      // Chama o método close para garantir que a conexão seja fechada de forma adequada
      close();
    }
  }
}
