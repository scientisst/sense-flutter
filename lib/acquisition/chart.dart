...
class Chart extends StatelessWidget {
  ...
  const Chart(this.x, this.y, {this.zeroBound = true});

  @override
  Widget build(BuildContext context) {
    final List<Series<int, DateTime>> series = <Series<int, DateTime>>[
      charts.Series<int, DateTime>(
        ...
        data: List.generate(x.length, (int index) => index),
      ),
    ];
    return charts.TimeSeriesChart(
      series,
      defaultRenderer: LineRendererConfig(strokeWidthPx: 3),
      animate: false,
      primaryMeasureAxis: charts.NumericAxisSpec(
        tickProviderSpec:
            zeroBound
                ? const StaticNumericTickProviderSpec(<TickSpec<num>>[
                  TickSpec<num>(0),
                  TickSpec<num>(512),
                  TickSpec<num>(1024),
                  TickSpec<num>(1536),
